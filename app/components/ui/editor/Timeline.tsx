...
import { SubtitleFragmentTrackItem } from "./SubtitleFragmentTrackItem";
...
export function Timeline({
    ...
    onSelectSubtitleIndex,
    activeTool,
}: TimelineProps) {
    const t = useTranslations("timeline");

    const showZoomTrack = zoomFragments.length > 0 || activeTool === "zoom";
    const showSubtitlesTrack = subtitleSegments.length > 0;
    const showAudioTrack = audioTracks.length > 0;

    const timelineHeight = useMemo(() => {
        let height = 28; // Ruler
        height += 64; // Video track
        if (showZoomTrack) height += 48; // Zoom track
        if (showSubtitlesTrack) height += 32; // Sub AI track
        if (showAudioTrack) height += 28; // Audio track
        return height + 8; // Extra padding
    }, [showZoomTrack, showSubtitlesTrack, showAudioTrack]);
    ...
    return (
        <div ref={containerRef} className="flex flex-col w-full relative select-none">
            <div 
                className="shrink-0 bg-[#0D0D11] border-t border-white/10 flex flex-col font-mono text-[10px] relative overflow-hidden"
                style={{ height: timelineHeight }}
            >
                <div className="flex-1 flex flex-row relative overflow-hidden h-full">
                    {/* Scrollable content container containing BOTH Sidebar and Tracks */}
                    <div
                        ref={trackRef}
                        data-scrollable
                        className="flex-1 flex overflow-x-auto overflow-y-auto custom-scrollbar relative"
                        style={{ scrollBehavior: 'smooth' }}
                    >
                        {/* Label sidebar (sticky left-0 inside scroll container) */}
                        <LabelSidebar
                            showZoomTrack={showZoomTrack}
                            showSubtitlesTrack={showSubtitlesTrack}
                            showAudioTrack={showAudioTrack}
                        />

                        {/* Tracks area */}
                        <div style={{ width: contentWidth > 0 ? contentWidth : '100%' }} className="relative flex flex-col h-full pl-2 shrink-0">
                            ...
                            <div className="flex-1 flex flex-col" onClick={handleTrackClick}>
                                ...
                                {/* Video track */}
                                <div className="h-16 shrink-0 flex items-center py-1.5 relative">
                                    <div
                                ...
                                {showZoomTrack && (
                                    <div
                                        className="h-12 shrink-0 flex items-center border-t border-white/5 relative"
                                        onMouseMove={(e) => {
                                ...
                                {/* Subtitles track - only show if there are subtitle segments */}
                                {subtitleSegments.length > 0 && (
                                    <div className="h-8 shrink-0 flex items-center border-t border-white/5 bg-purple-500/[0.01] relative">
                                        <div
                                            className="h-full flex items-center relative"
                                            style={{ width: contentWidth > 0 ? contentWidth : '100%' }}
                                        >
                                            {subtitleSegments.map((seg, idx) => (
                                                <SubtitleFragmentTrackItem
                                                    key={idx}
                                                    segment={seg}
                                                    index={idx}
                                                    isSelected={selectedSubtitleIndex === idx}
                                                    contentWidth={contentWidth}
                                                    videoDuration={validDuration}
                                                    otherSegments={subtitleSegments.filter((_, i) => i !== idx)}
                                                    onSelect={() => onSelectSubtitleIndex?.(idx)}
                                                    onUpdate={onUpdateSubtitleSegment || (() => {})}
                                                />
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {audioTracks.length > 0 && (
                                    <div className="h-10 shrink-0 flex items-center border-t border-white/5 relative">
                                        <div
                                            className="h-full flex items-center relative"
                                            style={{ width: contentWidth > 0 ? contentWidth : '100%' }}
                                        >
