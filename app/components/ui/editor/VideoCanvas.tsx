...
export const VideoCanvas = forwardRef<VideoCanvasHandle, VideoCanvasProps>(function VideoCanvas({
    ...
    useEffect(() => {
        ...
        if (imageUrlToLoad) {
            ...
            if (imageUrlToLoad.startsWith("editor-asset://")) {
                const assetId = imageUrlToLoad.replace("editor-asset://", "");
                import("@/lib/editor-assets-cache").then(async ({ getCachedAsset }) => {
                    if (!active) return;
                    ...
                    console.error("Failed to load custom image asset from IndexedDB:", err);
                });
            } else {
        ...
        for (const element of canvasElements) {
            if (element.type === "image") {
                ...
                if (!cache.has(imageElement.imagePath)) {
                    ...
                    if (imageElement.imagePath.startsWith("editor-asset://")) {
                        const assetId = imageElement.imagePath.replace("editor-asset://", "");
                        import("@/lib/editor-assets-cache").then(async ({ getCachedAsset }) => {
                            if (!active) return;
                            const cached = await getCachedAsset(assetId);
                            if (cached && cached.blob && active) {
                            ...
                            console.error(`Failed to load cached asset ${assetId}:`, err);
                        });
                    } else {
