...
const AIMenu = lazy(() => import("./AIMenu").then(mod => ({ default: mod.AIMenu })));
...
export function ControlPanel({
    ...
    return (
        <div className="relative w-full sm:w-[320px] h-screen bg-[#141417] border-r border-white/10 flex flex-col shrink-0" role="complementary" aria-label="Control panel">
            ...
            <div className={`flex-1 ${["ai", "audio", "videos"].includes(activeTool) ? "overflow-hidden flex flex-col" : "overflow-y-auto custom-scrollbar"}`}>
                ...
                {activeTool === "ai" && (
                    <Suspense fallback={<AudioMenuSkeleton />}>
                        <AIMenu
                            ...
                            onUpdateSubtitleSegment={onUpdateSubtitleSegment}
                            onAddAudioTrackAtTime={onAddAudioTrackAtTime}
