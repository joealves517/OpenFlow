...
import { formatToVTT } from "@/app/lib/aiUtils";
...
export default function Editor() {
    ...
    const [uploadedImages, setUploadedImages] = useState<string[]>(() => {
        if (typeof window !== 'undefined') {
            const stored = localStorage.getItem("VidFlow-uploaded-images");
            if (stored) {
                try {
    ...
    const [subtitleSegments, setSubtitleSegments] = useState<{ start: number; end: number; text: string }[]>([]);
    const [selectedSubtitleIndex, setSelectedSubtitleIndex] = useState<number | null>(null);

    const handleSubtitlesChange = useCallback((url: string | null, segments?: any[]) => {
        setSubtitlesUrl(url);
        if (segments) {
    ...
    const handleDeleteSubtitleSegment = useCallback((index: number) => {
        setSubtitleSegments((prev) => {
            const updated = prev.filter((_, i) => i !== index);
            if (updated.length > 0) {
                const vtt = formatToVTT(updated);
            ...
            if (videoDuration > 0) {
                localStorage.setItem(`VidFlow-subtitles-trans-${videoDuration}`, JSON.stringify(updated));
                localStorage.setItem(`VidFlow-subtitles-segments-${videoDuration}`, JSON.stringify(updated));
            }
            return updated;
        });
    }, [videoDuration]);

    const handleUpdateSubtitleSegment = useCallback((index: number, updates: Partial<{ start: number; end: number; text: string }>) => {
        setSubtitleSegments((prev) => {
            ...
            if (videoDuration > 0) {
                localStorage.setItem(`VidFlow-subtitles-trans-${videoDuration}`, JSON.stringify(sorted));
                localStorage.setItem(`VidFlow-subtitles-segments-${videoDuration}`, JSON.stringify(sorted));
            }
            return sorted;
        });
    }, [videoDuration]);

    const handleSelectSubtitleIndex = useCallback((index: number | null) => {
    ...
    const handleImageExport = useCallback(async (
        ...
        try {
            ...
            link.download = `VidFlowshot-${Date.now()}.${format}`;
            link.href = url;
            link.click();
    ...
    const addCanvasElement = useCallback((element: CanvasElement) => {
        ...
        // Transparent caching: If element is an uploaded image, fetch it and save the binary blob to IndexedDB
        if (element.type === "image" && (element.imagePath.startsWith("data:") || element.imagePath.startsWith("blob:"))) {
            fetch(element.imagePath)
                .then(res => res.blob())
                .then(blob => {
                    import("@/lib/editor-assets-cache")
                        .then(mod => mod.saveCachedAsset(element.id, blob))
                        .catch(err => console.warn("Failed to cache canvas image asset:", err));
                })
                .catch(err => console.warn("Failed to fetch binary data for canvas image:", err));
        }
    }, []);
    ...
    const deleteCanvasElement = useCallback((idOrIds: string | string[]) => {
        ...
        // Clean up corresponding cached image assets from IndexedDB asynchronously
        import("@/lib/editor-assets-cache").then(mod => {
            idsToDelete.forEach(id => mod.deleteCachedAsset(id).catch(e => console.warn("Failed to delete cached asset:", id, e)));
        }).catch(err => console.warn("Failed to import asset cache:", err));
    }, []);
    ...
    const handleAudioUpload = useCallback(async (file: File) => {
        try {
            ...
            if (audioTracks.length >= MAX_AUDIO_TRACKS) {
                gooeyToast.error("Audio tracks limit reached", { description: `Maximum of ${MAX_AUDIO_TRACKS} audio tracks allowed.` });
                return;
            }
            ...
            await new Promise<void>((resolve) => {
                ...
                // Fallback timeout after 1.2 seconds to prevent deadlocks in extension context
                setTimeout(done, 1200);
            });

            // Fallback duration estimation if metadata load fails or yields NaN
            const finalDuration = (audio.duration && !isNaN(audio.duration) && audio.duration > 0)
                ? audio.duration
                : Math.max(3, Math.round(file.size / 16000));
            ...
            // Persist the binary file into IndexedDB cache asynchronously
            import("@/lib/audio-upload-cache")
                .then(mod => mod.saveCachedAudio(newAudio.id, file, finalDuration))
                .catch(err => console.warn("Failed to save audio blob to IndexedDB cache:", err));
            ...
            // Automatically limit initial duration to fit the video if it exceeds it, enabling direct timeline insertion
            const trackDuration = videoDuration > 0 ? Math.min(finalDuration, videoDuration) : finalDuration;
            ...
            gooeyToast.error("Audio upload failed", { description: "Failed to upload the audio file. Please try again." });
        }
    ...
    const handleAudioDelete = useCallback((audioId: string) => {
        setUploadedAudios(prev => {
            ...
            if (audio) {
                ...
                // Also clean up from IndexedDB cache
                import("@/lib/audio-upload-cache")
                    .then(mod => mod.deleteCachedAudio(audioId))
                    .catch(err => console.warn("Failed to delete audio from cache:", err));
            }
    ...
    const handleAddAudioTrackAtTime = useCallback(async (file: File, startTime: number, duration: number) => {
        try {
            const url = URL.createObjectURL(file);
            const newAudio: import("@/types/audio.types").UploadedAudio = {
                id: `audio-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
            ...
            // Persist the AI Dubbing track binary into IndexedDB cache asynchronously
            import("@/lib/audio-upload-cache")
                .then(mod => mod.saveCachedAudio(newAudio.id, file, duration))
                .catch(err => console.warn("Failed to save AI Dubbing audio to cache:", err));
            ...
            console.error('Error adding tts audio track:', error);
        }
    }, []);
    ...
    const handleToggleMuteOriginalAudio = useCallback((mute?: boolean) => {
        setMuteOriginalAudio(prev => mute !== undefined ? mute : !prev);
    }, []);
    ...
    const handleExport = (quality: ExportQuality) => {
        ...
        let exportVideoBlob = videoBlob;
        if (!exportVideoBlob && videoClips.length === 1) {
            const firstClip = videoClips[0];
            exportVideoBlob = videoBlobsRef.current.get(firstClip.libraryVideoId) || null;
        }

        exportVideo({
            quality,
            videoBlob: exportVideoBlob ?? undefined,
    ...
    useEffect(() => {
        if (uploadedImages.length > 0) {
            localStorage.setItem("VidFlow-uploaded-images", JSON.stringify(uploadedImages));
        }
    }, [uploadedImages]);
    ...
    // Synchronize video player events (ended, pause) with background audio and playhead states
    useEffect(() => {
        ...
        const handleVideoEnded = () => {
            setIsPlaying(false);
            syncAudioPlayback(video.currentTime, false);
        };
    ...
    const handleImageUpload = useCallback(async (file: File) => {
        try {
            ...
            import("@/lib/editor-assets-cache")
                ...
                .catch(err => console.warn("Failed to cache background image asset:", err));

            setUploadedImages(prev => [blobUrl, ...prev]);
    ...
    const handleImageRemove = useCallback((url: string) => {
        ...
        // Clean up corresponding asset from IndexedDB cache
        const mapped = assetBlobMapRef.current.get(url);
        if (mapped) {
            assetBlobMapRef.current.delete(url);
            import("@/lib/editor-assets-cache")
                .then(mod => mod.deleteCachedAsset(mapped.id))
                .catch(err => console.warn("Failed to delete cached background image:", err));
        }
    }, [selectedImageUrl]);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if ((e.key === "Delete" || e.key === "Backspace") && selectedSubtitleIndex !== null) {
                e.preventDefault();
                handleDeleteSubtitleSegment(selectedSubtitleIndex);
    ...
    }, [selectedElementId, selectedZoomFragmentId, selectedAudioTrackId, selectedVideoClipId, selectedSubtitleIndex, deleteCanvasElement, handleDeleteZoomFragment, handleDeleteAudioTrack, handleDeleteVideoClip, handleDeleteSubtitleSegment, copySelectedElement, pasteElement, isPhotoMode, copiedElement, textToolActive]);
    ...
    const assetBlobMapRef = useRef<Map<string, { id: string, file: Blob }>>(new Map());
    const [isSessionRestored, setIsSessionRestored] = useState(false);

    // Robust local session auto-save for Video Mode
    useEffect(() => {
        if (videoDuration > 0 && isVideoMode && isSessionRestored) {
            // Transparently serialize RAM Blob URLs to editor-asset:// format to prevent QuotaExceeded errors
            const serializeUrl = (url: string) => {
                const mapped = assetBlobMapRef.current.get(url);
                return mapped ? `editor-asset://${mapped.id}` : url;
            };

            const serializedUploadedImages = uploadedImages.map(serializeUrl);
            const serializedSelectedImageUrl = selectedImageUrl ? serializeUrl(selectedImageUrl) : "";

            const serializedCanvasElements = canvasElements.map(el => {
                if (el.type === "image" && el.imagePath && (el.imagePath.startsWith("blob:") || el.imagePath.startsWith("data:"))) {
                    const mapped = assetBlobMapRef.current.get(el.imagePath);
                    const assetId = mapped ? mapped.id : el.id;
                    return { ...el, imagePath: `editor-asset://${assetId}` };
                }
                return el;
            });
            ...
            const sessionData = {
                ...
                muteOriginalAudio,
                masterVolume,
                uploadedImages: serializedUploadedImages,
                selectedImageUrl: serializedSelectedImageUrl
            };
            localStorage.setItem(`VidFlow-video-session-${videoDuration}`, JSON.stringify(sessionData));
        }
    }, [
        videoDuration,
        isVideoMode,
    ...
    // Robust local session auto-restore for Video Mode
    useEffect(() => {
        if (videoDuration > 0 && isVideoMode) {
            const saved = localStorage.getItem(`VidFlow-video-session-${videoDuration}`);
            if (saved) {
                try {
                    const session = JSON.parse(saved);
                    if (session.zoomFragments) setZoomFragments(session.zoomFragments);
                    if (session.audioTracks) setAudioTracks(session.audioTracks);
                    if (session.mockupId) setMockupId(session.mockupId);
                    if (session.mockupConfig) setMockupConfig(session.mockupConfig);
                    if (session.backgroundTab) setBackgroundTab(session.backgroundTab);
                    if (session.backgroundColorConfig) setBackgroundColorConfig(session.backgroundColorConfig);
                    if (session.shadows !== undefined) setShadows(session.shadows);
                    if (session.roundedCorners !== undefined) setRoundedCorners(session.roundedCorners);
                    if (session.padding !== undefined) setPadding(session.padding);
                    if (session.backgroundBlur !== undefined) setBackgroundBlur(session.backgroundBlur);
                    if (session.muteOriginalAudio !== undefined) setMuteOriginalAudio(session.muteOriginalAudio);
                    if (session.masterVolume !== undefined) setMasterVolume(session.masterVolume);

                    // Asynchronously restore binary assets (audios, backgrounds, canvas element images)
                    const restoreBinaryAssets = async () => {
                        try {
                            const { getCachedAsset } = await import("@/lib/editor-assets-cache");

                            // 1. Restore Background Images
                            if (session.uploadedImages && Array.isArray(session.uploadedImages)) {
                                const restoredImages = await Promise.all(
                                    session.uploadedImages.map(async (imgUrl: string) => {
                                        if (imgUrl.startsWith("editor-asset://")) {
                                            const assetId = imgUrl.replace("editor-asset://", "");
                                            const cached = await getCachedAsset(assetId);
                                            if (cached && cached.blob) {
                                                const newUrl = URL.createObjectURL(cached.blob);
                                                assetBlobMapRef.current.set(newUrl, { id: assetId, file: cached.blob });
                                                return newUrl;
                                            }
                                        }
                                        return imgUrl;
                                ...
                                // Restore selected background URL
                                if (session.selectedImageUrl) {
                                    if (session.selectedImageUrl.startsWith("editor-asset://")) {
                                        const assetId = session.selectedImageUrl.replace("editor-asset://", "");
                                        const cached = await getCachedAsset(assetId);
                                        if (cached && cached.blob) {
                                            const newUrl = URL.createObjectURL(cached.blob);
                                        ...
                                        setSelectedImageUrl(session.selectedImageUrl);
                                    }
                                }
                            }

                            // 2. Restore Canvas Sticker Images
                            if (session.canvasElements && Array.isArray(session.canvasElements)) {
                                const restoredElements = await Promise.all(
                                    session.canvasElements.map(async (el: any) => {
                                        if (el.type === "image" && el.imagePath && el.imagePath.startsWith("editor-asset://")) {
                                            const assetId = el.imagePath.replace("editor-asset://", "");
                                            const cached = await getCachedAsset(assetId);
                                            if (cached && cached.blob) {
                                                const newUrl = URL.createObjectURL(cached.blob);
                                                assetBlobMapRef.current.set(newUrl, { id: assetId, file: cached.blob });
                                                return { ...el, imagePath: newUrl };
                                            }
                                        }
                                        return el;
                                    })
                                );
                                setCanvasElements(restoredElements);
                            } else if (session.canvasElements) {
                                setCanvasElements(session.canvasElements);
                            }

                            // 3. Restore Audio Tracks
                            if (session.uploadedAudios && Array.isArray(session.uploadedAudios)) {
                                const { getCachedAudio } = await import("@/lib/audio-upload-cache");
                                const restoredList = await Promise.all(
                                    session.uploadedAudios.map(async (audioItem: any) => {
                                        try {
                                            const cached = await getCachedAudio(audioItem.id);
                                            if (cached && cached.blob) {
                                                const newUrl = URL.createObjectURL(cached.blob);
                                                return { ...audioItem, url: newUrl };
                                            }
                                        } catch (err) {
                                            console.warn("Failed to restore audio blob from IndexedDB cache:", audioItem.id, err);
                                        }
                                        return audioItem;
                                    })
                                );
                                setUploadedAudios(restoredList);
                            } else if (session.uploadedAudios) {
                                setUploadedAudios(session.uploadedAudios);
                            }

                        } catch (err) {
                            console.error("Failed to restore editor binary assets:", err);
                            if (session.canvasElements) setCanvasElements(session.canvasElements);
                            if (session.uploadedAudios) setUploadedAudios(session.uploadedAudios);
                        } finally {
                    ...
                    console.error("Error auto-restoring video session:", e);
                    setIsSessionRestored(true);
                }
            } else {
                setIsSessionRestored(true);
                            ...
                            >
                                <Suspense fallback={
                                    ...
                                    <ControlPanel
                                        ...
                                        onAddAudioTrackAtTime={handleAddAudioTrackAtTime}
                        ...
                        <>
                            ...
                            <Suspense fallback={<TimelineSkeleton />}>
                                <Timeline
                                    ...
                                    onSelectSubtitleIndex={handleSelectSubtitleIndex}
                                    activeTool={activeTool}
                                />
                            </Suspense>
                        </>
